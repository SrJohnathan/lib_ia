use crate::agents::{AgentRole, ConfigAgent};
use crate::tools::{self, BASE_TOOLS};
...
impl SubagentConfig {
    ...
    pub fn is_valid(&self) -> Result<(), String> {
        ...
        {
            return Err(format!(
                "nome invalido '{}': use apenas letras, numeros, '_', '-' ou '.'",
                self.name
            ));
        }
...
pub fn read_agent(name: &str, project_dir: &Path) -> Result<String, String> {
    ...
    Err(format!(
        "agente '{}' nao encontrado (procurei por {})",
        name, file_name
    ))
}
